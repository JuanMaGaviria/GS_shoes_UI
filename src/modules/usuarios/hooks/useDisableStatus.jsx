    ...
    const toggleStatus = async (id, currentStatus) => {
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (USE_BACKEND) {
                    // Lógica para backend
                    const response = await api.patch(`usuarios/actualizar/${id}/`);
                    newStatus = response.data.is_active;
                } else {
                    // Simular delay de API para datos ficticios
                    await new Promise(resolve => setTimeout(resolve, 600));

                    // Buscar el usuario en los datos ficticios
                    const usuarioIndex = usuariosFicticios.results.findIndex(u => u.id === id);
                    ...
                    newStatus = !usuariosFicticios.results[usuarioIndex].is_active;
                    usuariosFicticios.results[usuarioIndex].is_active = newStatus;
                    usuariosFicticios.results[usuarioIndex].updated_at = new Date().toISOString();
